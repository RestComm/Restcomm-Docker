...
if [ -n "$NEXMO_SMPP_TYPE" ]; then
  ...
  sed -i "s/SMPP_ACTIVATE=.*/SMPP_ACTIVATE='true'/" $BASEDIR/bin/restcomm/smpp.conf
  sed -i "s/SMPP_SYSTEM_ID=.*/SMPP_SYSTEM_ID='${DID_LOGIN}'/" $BASEDIR/bin/restcomm/smpp.conf
  sed -i "s/SMPP_PASSWORD=.*/SMPP_PASSWORD='${DID_PASSWORD}'/" $BASEDIR/bin/restcomm/smpp.conf
  sed -i "s/SMPP_SYSTEM_TYPE=.*/SMPP_SYSTEM_TYPE='${NEXMO_SMPP_TYPE}'/" $BASEDIR/bin/restcomm/smpp.conf
...
if [ -n "$MEDIASERVER_LOGS_LOCATION" ]; then
  ...
  mkdir -p "${MEDIASERVER_LOGS_LOCATION}"
...
if [ -n "$RESTCOMMHOST" ]; then
  ...
  sed -i "s|<hostname>.*<\/hostname>|<hostname>${RESTCOMMHOST}<\/hostname>|" $BASEDIR/standalone/deployments/restcomm.war/WEB-INF/conf/restcomm.xml
else
  sed -i "s|<hostname>.*<\/hostname>|<hostname>${STATIC_ADDRESS}<\/hostname>|" $BASEDIR/standalone/deployments/restcomm.war/WEB-INF/conf/restcomm.xml
 fi
